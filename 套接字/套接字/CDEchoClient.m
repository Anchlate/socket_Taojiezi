...
- (instancetype)initWithAddress:(NSString *)address port:(int)port {
    ...
    if (!self.socket) {
        ...
        memset(&servaddr, 0, sizeof(servaddr));// 初始化服务器地址结构体的值为0
        servaddr.sin_len = sizeof(servaddr);
        servaddr.sin_family = AF_INET;
        servaddr.sin_port = htons(port);
        // 将字符串形式的地址转换成网络地址的结构体变量
        int inet = inet_pton(AF_INET, [address cStringUsingEncoding:NSUTF8StringEncoding], &servaddr.sin_addr);
        NSLog(@"......inet:%d", inet);
        ...
        CFSocketError cfsocketerror = CFSocketConnectToAddress(self.socket, connectAddr, kConnectionTimeout) != kCFSocketSuccess;
        
        if (!connectAddr || cfsocketerror != kCFSocketSuccess) {
